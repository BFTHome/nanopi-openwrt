...
### 下载地址：
https://github.com/bfthome/nanopi-openwrt/releases  
(img.gz档不需要解压，可以直接使用刷机工具刷入)

### 使用提示：
默认用户名是root, 密码是password，局域网IP为192.168.2.1  
烧制完固件插入tf卡并启动完成，电脑端显示“网络（已连接）”之后，在浏览器输入 http://openwrt/ 可以直接打开路由器后台，无需修改本地连接设置或者查看IP地址。  
如果网络状态一直是未识别（上电超过5分钟），请直接插拔一次电源重启试试。

### 终端内在线升级方法：
```bash
wget -qO- https://github.com/bfthome/nanopi-openwrt/raw/master/scripts/autoupdate.sh | sh
```
slim版
```bash
wget -qO- https://github.com/bfthome/nanopi-openwrt/raw/master/scripts/autoupdate.sh | ver=-slim sh
```
(脚本由gary lau提供，非常感谢！)
...
### 更新说明：
https://github.com/bfthome/nanopi-openwrt/blob/master/CHANGELOG.md
